...
The following file combines the force plate and injury datasets into one large .csv for future analysis. It uses "CMJ_All_De_Identified.csv" found in the Google Drive and "injury_data.csv" found in both Google Drive and github. We removed any extraneous variables and created our dependent variable "Injured" which indicates a jump occurring at least 4 weeks prior to an athlete injury. The final .csv after combining is "final_data.csv" and can be found in the Google Drive, not the github repo, due to its large size. 
...
# checking ratios
table(merged_data_2$Injured)
sum(is.na(merged_data_2$days_before))
```

Using 2 weeks, we have 761 injured cases vs. 25,193 non-injured cases, a large imbalance. We also see that days_before produced 15803 NA values, meaning these were the total jumps from non-injured athletes. We will now extend that window to 4 weeks to see if we capture more injured cases and slower-developing fatigue patterns. 
...
sum(is.na(merged_data_4$days_before))
```

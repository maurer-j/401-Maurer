...
Now, we need to remove identifiers and non-numeric columns. These identifiers include organizations, dates, and injury types. We will remove all character variables as well as "days_before" which was a helper variable to identify injured vs. non-injured jumps.  
...
```{r}
# performing an 80/20 split by athlete ID for train and test so that each athlete appears only once (avoiding info leak) (Sanderson, 2024)
ids <- unique(data$ID)
...
```{r}
# median imputation (helped to debug with ChatGPT)
...
```{r}
# lock folds so results don't change (helped debug from ChatGPT to ensure reproducibility)
K <- 10
set.seed(42)  
...
After weighting the training set, we received alpha of 0.2, but it still leans towards Ridge rather than Lasso, so we'll have limited feature selection.  
...
set.seed(42)
K <- 10
foldid <- sample(rep(1:K, length.out = length(y_train))) # ensures that the same observations go into the same fold each time so we don't do random splits each time
...
```{r}
# coefficients and selected features at λ_min and λ_1se 
...
```{r}
# test-set performance: λ_min vs λ_1se
...
# remove perfect linear combinations (exact multicollinearity since we had this error before) (helped debug with ChatGPT)
...
select(Injured, all_of(lasso_vars_final))

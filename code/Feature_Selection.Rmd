...
fig1 <- ggplot(injury_counts, aes(x = Class, y = Proportion, fill = Class)) +
  ...
  scale_fill_manual(values = c("Non-Injured" = "gray80", "Injured" = "#D02130")) +
  scale_y_continuous(
...
# 8.4 Fit a simple linear model (only to compute VIFs)
...
```{r}
# plotting VIFs as a bar chart
vif_df <- data.frame(
  Variable = names(vifs_sorted),
  VIF = as.numeric(vifs_sorted)
)

top_vif <- head(vif_df, 15)

ggplot(top_vif, aes(x = reorder(Variable, VIF), y = VIF)) +
  geom_bar(stat = "identity", fill = "#D02130") +
  coord_flip() +
  labs(
    title = "Top 15 Variables by VIF",
    subtitle = "High multicollinearity persists among Lasso-selected CMJ metrics",
    x = "Variable",
  ...
  theme(
    ...
    plot.subtitle = element_text(color = "gray40", size = 8),  # smaller subtitle
    axis.title = element_text(face = "bold", size = 10)
  )
ggsave("vif_top15.png", width = 6, height = 4, dpi = 300)
```
...
# 7. Writing a New .csv With Lasso (Weighted) Selected Variables

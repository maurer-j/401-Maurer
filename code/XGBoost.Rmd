...
Now, we spit into training and test using stratified random sampling on the injured variable. We want to make sure that the training and test sets have the correct proportion (approximately 6%) of injured jumps. 
...
We split the data into a training (80%) and testing (20%) set using stratified random sampling (preserving the proportion of injured and non-injured cases). To do so, we will use the caret package and the createDataPartition function found in an online R tutorial written by Deepanshu Bhalla from listendata.com.
...
In the training data, only 1,103 out of all training jumps are injury events. So, each injured jump will be treated as roughly 18 times more important than non-injured jumps during model training. This balances the gradient so the model pays attention to both classes equally. The following table was debugged and helped generate with ChatGPT. 
...
# view the LaTeX code in console to paste into Overleaf
cat(latex_table)
```
...
We will now using a grid search cross-validation approach to find the best hyperparameters using the scaled training set. Information was found at https://medium.com/@heyamit10/xgboost-in-r-a-practical-guide-f14b722866c1 and debugged using ChatGPT.  

The grid search runs through all possible combination of candidate values for the following selection of parameters (https://carpentries-incubator.github.io/r-ml-tabular-data/06-Exploration/index.html):
...
We will now loop through the grid. For each configuration we run 5-fold cross-validation on the training set, optimize AUC, and use early stopping to pick the best number of trees (nrounds). We will keep the best model seen so far. ChatGPT was used to adapt the following loop into a more advanced process that prints out the parameters at each fold so that we could better understanding what was occurring during each round. 

```{r}
set.seed(42)
K <- 5
folds <- createFolds(y = y_train, k = K, list = TRUE, returnTrain = FALSE)

# grid search with the use of ChatGPT to help print messages during each run through
...
for (i in seq_len(nrow(param_grid))) {
  ...
  cv <- tryCatch(
    xgb.cv(
      params   = params,
      data     = dtrain,
      nrounds  = 2000,                 # high cap but we use early stopping to find the best
      folds    = folds,                
      early_stopping_rounds = 50, # kind of randome but wanted to be large enough to capture everything without being too exhaustive
      stratified = TRUE,
      verbose  = 0, # does not print every little thing
      seed     = 42                    # extra  to ensure reproducibility
    ),
    error = function(e) {
...
Code was developed from xgb.train Rdocumentation as well as Kaggle to understand the "watchlist" parameter. 

```{r}
# build cv_results 
cv_results <- bind_rows(cv_summaries[!sapply(cv_summaries, is.null)])

# making sure it runs
stopifnot(is.data.frame(cv_results), nrow(cv_results) > 0)

# finding the row that maximizes AUC
best_row <- cv_results[which.max(cv_results$best_auc), ]

# build parameter list from the best row and the fixed constants like lambda and alpha
best_params <- list(
  objective        = "binary:logistic",
  eval_metric      = "auc",
  eta              = best_row$eta,
...
We will now build the ROC curve and compute AUC score using the pROC package. 

```{r}
# compute ROC
roc_obj <- roc(response = y_test, predictor = test_pred_prob)

# AUC
auc_value <- auc(roc_obj)

# ROC curve plot --> used ChatGPT to help polish graph
...
# for each threshold, calculate precision, recall, F1 --> debugged with ChatGPT
...
# create results table --> helped translate into Table and LaTex Code with ChatGPT
...
# view the LaTeX code for easy copying into Overleaf
cat(results_latex)
```
...
# add thousands separators (created with ChatGPT)
...
We will now rank features by how influential they are overall, ignoring positive or negative influence on injury risk by taking the mean absolute SHAP (helped to debug with ChatGPT). 
